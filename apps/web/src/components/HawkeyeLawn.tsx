...
// ─── LIVE INFRASTRUCTURE ─────────────────────────────────────
// Stripe Account: acct_1R0mI1CHq2ueHUIS (Oakland Investment Group LLC)
// Cloudflare D1:  174d9cc0-0a9a-4ed4-ad7b-34c54c1821ff (hawkeyelawn-db)
// Cloudflare KV:  7dd359bdc70049ee85498f5be04c9330 (AUTH_STORE)

const STRIPE_LINKS = {
  small:  "https://buy.stripe.com/00w28qfTy3xu0dy3Vl0oM07",  // $35 - Small Yard
  medium: "https://buy.stripe.com/3cIdR88r63xu3pKdvV0oM08",  // $65 - Medium Yard
  large:  "https://buy.stripe.com/fZudR88r68RO3pKbnN0oM09",  // $95 - Large Yard
  xl:     "https://buy.stripe.com/8x27sKfTy4Byf8s2Rh0oM0a",  // $125 - XL Yard
};
...
const YARD_SIZES = [
  { id:"small",  label:"Small",  range:"< 5,000 sq ft",       sqft:2500,  price:35,  icon:"🏡" },
  { id:"medium", label:"Medium", range:"5,000–10,000 sq ft",  sqft:7500,  price:65,  icon:"🏠" },
  { id:"large",  label:"Large",  range:"10,000–20,000 sq ft", sqft:15000, price:95,  icon:"🏘️" },
  { id:"xl",     label:"XL",     range:"> 20,000 sq ft",      sqft:25000, price:125, icon:"🌾" },
];
...
.btn{background:#FFCD00;color:#000;border:none;font-family:'Barlow Condensed',sans-serif;font-weight:900;font-size:20px;
     letter-spacing:1px;padding:18px;border-radius:4px;cursor:pointer;text-transform:uppercase;
     transition:all .15s;width:100%;display:block;text-align:center;}
.btn:hover{background:#FFD700;transform:scale(1.015);}
.btn:disabled{opacity:.3;cursor:not-allowed;transform:none;}
.btn-out{background:transparent;color:#FFCD00;border:2px solid #FFCD00;font-family:'Barlow Condensed',sans-serif;
         font-weight:700;font-size:16px;padding:13px;border-radius:4px;cursor:pointer;text-transform:uppercase;
         transition:all .15s;width:100%;}
.btn-out:hover{background:#FFCD0018;}
.btn-crypto{background:#0a0800;color:#FFCD00;border:2px solid #FFCD00;font-family:'Barlow Condensed',sans-serif;
            font-weight:900;font-size:20px;padding:18px;border-radius:4px;cursor:pointer;text-transform:uppercase;
            transition:all .15s;width:100%;}
.btn-crypto:hover{background:#1a1200;}
input{background:#111;border:2px solid #222;border-radius:6px;color:white;font-family:'Barlow',sans-serif;
      font-size:16px;padding:14px 16px;width:100%;outline:none;transition:border-color .2s;}
input:focus{border-color:#FFCD00;}
input::placeholder{color:#444;}
.pay-opt{background:#111;border:2px solid #222;border-radius:10px;padding:20px;cursor:pointer;
...
function Pulse({ color="#22c55e" }) {
  return <span style={{ display:"inline-block", width:9, height:9, borderRadius:"50%", background:color, marginRight:7, animation:"pulse-ring 1.4s infinite" }} />;
}

function StepBar({ current }) {
  const steps = ["Address","Yard & Service","Payment","Done"];
  return (
      ...
      {steps.map((s, i) => (
        <div key={s} style={{ display:"flex", alignItems:"center", flex:i<steps.length-1?1:0 }}>
          <div style={{
            ...
            background:i<=current?GOLD:"#1a1a1a",
            border:`2px solid ${i<=current?GOLD:"#333"}`,
            display:"flex", alignItems:"center", justifyContent:"center",
            fontSize:11, fontWeight:900, color:i<=current?BLACK:"#555",
            flexShrink:0, transition:"all .3s",
            boxShadow:i===current?`0 0 16px ${GOLD}66`:"none",
          }}>
            {i<current?"✓":i+1}
          </div>
          {i<steps.length-1 && (
            <div style={{ flex:1, height:2, margin:"0 4px", background:i<current?GOLD:"#1e1e1e", transition:"background .5s" }} />
          )}
        </div>
      ))}
    </div>
...
function CryptoModal({ price, onClose, onPaid }) {
  const [coin, setCoin] = useState("USDC");
  const [stage, setStage] = useState("select"); // select | waiting | confirmed
      ...
      <div style={{ background:"#0a0a0a", border:`2px solid ${GOLD}`, borderRadius:14, padding:28, width:"100%", maxWidth:440, animation:"slideUp .3s ease", maxHeight:"90vh", overflowY:"auto" }}>
        ...
        {stage === "select" && (
          <>
            ...
            <div style={{ fontWeight:800, fontSize:12, textTransform:"uppercase", color:"#666", letterSpacing:2, marginBottom:12 }}>Select Currency</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:20 }}>
              {CRYPTO_COINS.map(c => (
                <div key={c.id} onClick={() => setCoin(c.id)} style={{
                  background: coin===c.id ? `${c.color}22` : "#111",
                  border:`2px solid ${coin===c.id ? c.color : "#222"}`,
                  borderRadius:8, padding:"14px 8px", textAlign:"center", cursor:"pointer", transition:"all .15s"
                }}>
            ...
            <div style={{ background:"#111", borderRadius:8, padding:16, marginBottom:20 }}>
              <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#555", marginBottom:8 }}>Send {coin} to this address:</div>
              <div style={{ fontFamily:"monospace", fontSize:11, color:selectedCoin?.color, wordBreak:"break-all", background:"#0a0a0a", padding:10, borderRadius:6, border:"1px solid #222" }}>
                {mockAddr[coin]}
              </div>
              <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:11, color:"#444", marginTop:8 }}>
                Powered by Coinbase Commerce · Auto-converts to USD for contractor payout
              </div>
            </div>

            <button className="btn-crypto" onClick={handlePay}>CONFIRM PAYMENT IN {coin} →</button>
            <p style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#444", textAlign:"center", marginTop:8 }}>
              BTC · ETH · USDC · SOL · DOGE all accepted
            </p>
          </>
        )}
        ...
        {stage === "waiting" && (
          <div style={{ textAlign:"center", padding:"32px 0" }}>
            ...
            <div style={{ fontFamily:"'Barlow', sans-serif", color:"#666", fontSize:14 }}>
              Waiting for blockchain confirmation.<br />This usually takes under 30 seconds.
            </div>
          </div>
        )}
        ...
        {stage === "confirmed" && (
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:32, fontWeight:900, textTransform:"uppercase", marginBottom:8 }}>
              <span style={{ color:GOLD }}>PAYMENT CONFIRMED</span>
            </div>
            <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:13, color:"#555", marginBottom:6 }}>Transaction Hash:</div>
            <div style={{ fontFamily:"monospace", fontSize:11, color:"#22c55e", wordBreak:"break-all", background:"#0a0a0a", padding:10, borderRadius:6, border:"1px solid #1a3a1a", marginBottom:20 }}>
              {txHash}
            </div>
...
export default function HawkeyeLawn() {
  ...
  if (proView) return <ProDashboard onBack={() => setProView(false)} jobLive={jobLive} claimed={claimed} setClaimed={setClaimed} />;
  ...
  return (
    <div style={{ minHeight:"100vh", background:BLACK, color:"white", fontFamily:"'Barlow Condensed', Impact, sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <nav style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 20px", borderBottom:"1px solid #111" }}>
        ...
        <button className="btn-out" style={{ width:"auto", fontSize:13, padding:"8px 16px" }} onClick={() => setProView(true)}>PRO LOGIN</button>
      </nav>
      ...
      <div style={{ flex:1, maxWidth:500, margin:"0 auto", width:"100%", padding:"24px 20px 52px" }}>
        ...
        {step === 0 && (
          <div className="slide">
            <div style={{ textAlign:"center", padding:"36px 0 44px" }}>
              <div style={{ fontSize:12, color:GOLD, fontWeight:700, letterSpacing:3, marginBottom:14, textTransform:"uppercase" }}>
                <Pulse /> PROS AVAILABLE NOW · CEDAR RAPIDS
              </div>
              <h1 style={{ fontSize:64, fontWeight:900, lineHeight:.92, textTransform:"uppercase", marginBottom:14 }}>
                GET MOWED<br /><span style={{ color:GOLD }}>TODAY.</span>
              </h1>
              <p style={{ fontFamily:"'Barlow', sans-serif", fontSize:16, color:"#777", marginBottom:14 }}>
                Lawn care at Iowa Speed. Book in 60 seconds.<br />No account. No BS.
              </p>
            ...
            {[
              {n:"01",t:"Enter Your Address",d:"Google Places finds your home instantly. No account needed."},
              {n:"02",t:"Pick Yard + Service",d:"Instant price shown. Zero surprises or hidden fees."},
              {n:"03",t:"Pay — Card or Crypto",d:"BTC, ETH, SOL, USDC, DOGE, or card. All accepted."},
              {n:"04",t:"Pro Claims & Arrives",d:"First available contractor gets it. You get a text when they leave."},
            ].map(({n,t,d})=>(
              <div key={n} style={{ display:"flex", gap:14, marginBottom:18 }}>
                ...
                <div>
                  <div style={{ fontWeight:800, fontSize:17, textTransform:"uppercase" }}>{t}</div>
                  <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:14, color:"#666" }}>{d}</div>
                </div>
              </div>
            ))}
        ...
        {step === 1 && (
          <div className="slide">
            ...
            <p style={{ fontFamily:"'Barlow', sans-serif", color:"#666", marginBottom:22, fontSize:14 }}>Confirm same-day availability in your zip code.</p>
            <div style={{ position:"relative", marginBottom:14 }}>
              <input placeholder="Start typing your Iowa address..." value={address} onChange={e=>{ setAddress(e.target.value); setSuggestions(e.target.value.length>2?FAKE_ADDRESSES.filter(a=>a.toLowerCase().includes(e.target.value.toLowerCase())).slice(0,4):[]); }} autoFocus />
              {suggestions.length>0 && (
                <div style={{ position:"absolute", top:"100%", left:0, right:0, background:"#161616", border:"1px solid #2a2a2a", borderTop:"none", borderRadius:"0 0 6px 6px", zIndex:20 }}>
                  {suggestions.map(s=>(
                    <div key={s} onClick={()=>{ setAddress(s); setSuggestions([]); }} style={{ padding:"12px 16px", cursor:"pointer", fontFamily:"'Barlow', sans-serif", fontSize:14, borderBottom:"1px solid #1a1a1a", transition:"background .15s" }}
                      onMouseEnter={e=>e.currentTarget.style.background="#1a1600"}
            ...
            {!address && FAKE_ADDRESSES.slice(0,3).map(a=>(
              <div key={a} onClick={()=>setAddress(a)} style={{ padding:"10px 14px", background:"#0d0d0d", border:"1px solid #1a1a1a", borderRadius:6, marginBottom:8, cursor:"pointer", fontFamily:"'Barlow', sans-serif", fontSize:13, color:"#666", transition:"all .15s" }}
            ...
            <div style={{ height:18 }} />
            <button className="btn" disabled={!address} onClick={()=>address&&setStep(2)}>CONFIRM ADDRESS →</button>
        ...
        {step === 2 && (
          <div className="slide">
              ...
              {SERVICES.map(s=>(
                <div key={s.id} className={`card${svc===s.id?" on":""}`} onClick={()=>setSvc(s.id)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    ...
                    <div>
                      <div style={{ fontWeight:800, fontSize:17, textTransform:"uppercase" }}>{s.label}</div>
                      <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#666" }}>{s.desc}</div>
                    </div>
                  </div>
            ...
            <button className="btn" disabled={!yard} onClick={()=>yard&&setStep(3)}>CHOOSE PAYMENT →</button>
        ...
        {step === 3 && (
          <div className="slide">
            ...
            <p style={{ fontFamily:"'Barlow', sans-serif", color:"#555", fontSize:13, marginBottom:22 }}>Both options are instant, secure, and confirmed immediately.</p>
            ...
            <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:8, padding:18, marginBottom:24 }}>
              ...
              {[["📍 Address",address.split(",")[0]],["📐 Yard",YARD_SIZES.find(y=>y.id===yard)?.label],["🌿 Service",SERVICES.find(s=>s.id===svc)?.label]].map(([l,v])=>(
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:24 }}>
              <div className={`pay-opt${payMethod==="stripe"?" on":""}`} onClick={()=>setPayMethod("stripe")}>
                ...
                <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#666" }}>Visa · MC · Amex via Stripe</div>
              </div>
              <div className={`pay-opt${payMethod==="crypto"?" on":""}`} onClick={()=>setPayMethod("crypto")}>
                ...
                <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#666" }}>BTC · ETH · SOL · USDC · DOGE</div>
              </div>
            </div>

            {payMethod === "stripe" && (
              <div style={{ animation:"slideUp .3s ease" }}>
                <a href={STRIPE_LINKS[yard]} target="_blank" rel="noreferrer" style={{ textDecoration:"none" }}>
                  <button className="btn glow" style={{ fontSize:22 }} onClick={()=>setTimeout(()=>setStep(4),500)}>
                    💳 PAY ${price} WITH CARD →
                  </button>
                </a>
                <p style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#444", textAlign:"center", marginTop:8 }}>
                  Stripe Checkout · 256-bit SSL · acct_1R0mI1CHq2ueHUIS
                </p>
              </div>
            )}
            {payMethod === "crypto" && (
              <div style={{ animation:"slideUp .3s ease" }}>
                ...
                <p style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#444", textAlign:"center", marginTop:8 }}>
                  Coinbase Commerce · Non-custodial · Auto-converts for pro payout
                </p>
              </div>
            )}
            {!payMethod && <button className="btn" disabled>SELECT A PAYMENT METHOD ABOVE</button>}
            <button className="btn-out" style={{ marginTop:12 }} onClick={()=>setStep(2)}>← BACK</button>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="slide">
            ...
            <div style={{ textAlign:"center", paddingTop:12 }}>
              ...
              <h2 style={{ fontSize:46, fontWeight:900, textTransform:"uppercase", marginBottom:8 }}>
                PAYMENT<br /><span style={{ color:GOLD }}>CONFIRMED!</span>
              </h2>
              <p style={{ fontFamily:"'Barlow', sans-serif", color:"#666", marginBottom:30 }}>
                Your job is live. Notifying pros in your area now.
              </p>
              ...
              {!jobLive ? (
                ...
                <div style={{ marginBottom:28, animation:"slideUp .4s ease" }}>
                  <div style={{ fontWeight:800, fontSize:12, textTransform:"uppercase", color:"#22c55e", letterSpacing:2, marginBottom:14, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                    <Pulse /> 3 PROS NOTIFIED — FIRST TO CLAIM GETS YOUR JOB
                  </div>
                  {CONTRACTORS.map(c=>(
                    <div key={c.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:8, padding:14, marginBottom:10, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#666" }}>⭐ {c.rating} · {c.jobs} jobs · ETA {c.eta}</div>
                        </div>
                      </div>
              ...
              <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:8, padding:20 }}>
                <div style={{ fontWeight:800, fontSize:12, textTransform:"uppercase", letterSpacing:2, color:"#666", marginBottom:12 }}>📱 GET TEXTS WHEN PRO DEPARTS</div>
                <input placeholder="Your phone number" value={phone} onChange={e=>setPhone(e.target.value)} style={{ marginBottom:12 }} />
                <button className="btn" style={{ fontSize:16 }}>SEND ME UPDATES</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{ borderTop:"1px solid #0e0e0e", padding:"14px 20px", display:"flex", justifyContent:"space-between", fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#333" }}>
        <span>© 2025 HawkeyeLawn.com · Cedar Rapids, Iowa</span>
        <span>Privacy · Terms</span>
      </div>

      {cryptoOpen && <CryptoModal price={price} onClose={()=>setCryptoOpen(false)} onPaid={()=>{ setCryptoOpen(false); setStep(4); }} />}
    </div>
  );
}

// ─── PRO DASHBOARD ───────────────────────────────────────────
function ProDashboard({ onBack, jobLive, claimed, setClaimed }) {
  const [tab, setTab] = useState("leads");

  const LIVE_JOBS = jobLive ? [
    { id:"job_001", address:"1842 Oakwood Dr NE", zip:"52402", size:"Medium", service:"Full Service", payout:93.50, ago:"Just now", payMethod:"Stripe" },
    { id:"job_002", address:"4210 Prairie Ridge Rd", zip:"52302", size:"Large", service:"Mow + Trim",  payout:93.50, ago:"2 min ago", payMethod:"Bitcoin ₿" },
  ] : [];
  ...
  return (
    <div style={{ minHeight:"100vh", background:BLACK, color:"white", fontFamily:"'Barlow Condensed', Impact, sans-serif" }}>
      <nav style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 20px", borderBottom:"1px solid #111" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:11, color:"#444" }}>Jake M. · Cedar Rapids · 52402, 52403</div>
          </div>
        </div>
        <button onClick={onBack} style={{ background:"none", border:"none", color:"#444", cursor:"pointer", fontFamily:"'Barlow', sans-serif", fontSize:13 }}>← Exit</button>
      </nav>
      ...
      <div style={{ background:"#060f06", padding:"11px 20px", display:"flex", justifyContent:"space-between" }}>
        ...
        <span style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#444" }}>ZIP 52402 · 52403</span>
      </div>
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #0e0e0e" }}>
        {[["leads","LEADS"],["jobs","MY JOBS"],["pay","PAYOUTS"]].map(([id,label])=>(
          <div key={id} onClick={()=>setTab(id)} style={{ padding:"12px 20px", cursor:"pointer", fontWeight:700, fontSize:15, letterSpacing:1, borderBottom:`3px solid ${tab===id?GOLD:"transparent"}`, color:tab===id?GOLD:"#444", transition:"all .15s" }}>
            {label}
            {id==="leads" && LIVE_JOBS.length>0 && (
              <span style={{ marginLeft:6, background:"#22c55e", color:BLACK, borderRadius:"50%", width:18, height:18, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:900 }}>{LIVE_JOBS.length}</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:20 }}>
        {tab === "leads" && (
          LIVE_JOBS.length===0 ? (
            <div style={{ textAlign:"center", padding:"60px 20px" }}>
              ...
              <div style={{ fontFamily:"'Barlow', sans-serif", color:"#555", fontSize:14 }}>Leads appear here instantly when homeowners pay. Stay online!</div>
            </div>
          ) : LIVE_JOBS.map(job=>(
            <div key={job.id} style={{ background:"#060f06", border:"2px solid #22c55e", borderRadius:10, padding:20, marginBottom:14, animation:"newJob 2.5s infinite, slideUp .4s ease" }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
                <div>
                  ...
                  <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:13, color:"#666" }}>{job.size} Yard · {job.service} · Paid via {job.payMethod}</div>
                </div>
              ...
              {claimed===job.id ? (
                <div style={{ background:"#22c55e", color:BLACK, borderRadius:6, padding:14, textAlign:"center", fontWeight:900, fontSize:18, textTransform:"uppercase" }}>
                  ✅ JOB CLAIMED — HOMEOWNER DETAILS SENT
                </div>
              ) : (
                <button style={{ background:"#22c55e", color:BLACK, border:"none", borderRadius:6, padding:16, width:"100%", fontFamily:"'Barlow Condensed', sans-serif", fontWeight:900, fontSize:22, cursor:"pointer", textTransform:"uppercase", letterSpacing:1 }}
                  onClick={()=>setClaimed(job.id)}>
                  ⚡ CLAIM THIS JOB — ${job.payout}
                </button>
              )}
            </div>
          ))
        ...
        {tab === "jobs" && (
          [
            {addr:"933 15th St SE",         status:"Completed", earned:59.50, date:"Today",    method:"Stripe"},
            {addr:"711 Blairs Ferry Rd NE", status:"Completed", earned:85.00, date:"Yesterday",method:"Bitcoin ₿"},
            {addr:"2200 Edgewood Rd SW",    status:"Completed", earned:42.50, date:"Feb 17",   method:"USDC"},
          ].map((j,i)=>(
            <div key={i} style={{ background:"#111", border:"1px solid #1a1a1a", borderRadius:8, padding:16, marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ fontWeight:800, fontSize:16 }}>📍 {j.addr}</div>
                <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#555" }}>{j.date} · {j.status} · {j.method}</div>
              </div>
        ...
        {tab === "pay" && (
          <div style={{ animation:"slideUp .3s ease" }}>
            <div style={{ background:"#0d0d00", border:`2px solid ${GOLD}`, borderRadius:10, padding:24, marginBottom:20, textAlign:"center" }}>
              ...
              <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:13, color:"#444" }}>312 jobs completed</div>
            </div>

            <div style={{ background:"#111", border:"1px solid #1a1a1a", borderRadius:10, padding:20, marginBottom:14 }}>
              <div style={{ fontWeight:800, fontSize:12, textTransform:"uppercase", color:"#555", letterSpacing:2, marginBottom:16 }}>PAYOUT SPLIT</div>
              {[["Your Earnings","85%",GOLD],["Platform Fee","15%","#2a2a2a"]].map(([l,p,c])=>(
                <div key={l} style={{ marginBottom:14 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", fontFamily:"'Barlow', sans-serif", marginBottom:6, fontSize:15 }}>
                    <span style={{ color:"#aaa" }}>{l}</span><span style={{ color:c===GOLD?GOLD:"#555", fontWeight:700 }}>{p}</span>
                  </div>
                  <div style={{ height:8, background:"#1a1a1a", borderRadius:4, overflow:"hidden" }}>
                    <div style={{ height:"100%", background:c, width:p, borderRadius:4 }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:"#111", border:"1px solid #1a1a1a", borderRadius:10, padding:20, marginBottom:14 }}>
              <div style={{ fontWeight:800, fontSize:12, textTransform:"uppercase", color:"#555", letterSpacing:2, marginBottom:14 }}>PAYOUT OPTIONS</div>
              {[["💳 Stripe Connect","Instant to your bank · via Stripe"],["₿ Bitcoin Wallet","Self-custodial · 15-min settlements"],["◎ Solana Wallet","Sub-cent fees · instant finality"]].map(([t,d])=>(
                <div key={t} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px solid #161616" }}>
                  <div>
                    <div style={{ fontWeight:700, fontSize:15 }}>{t}</div>
                    <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#555" }}>{d}</div>
                  </div>
                  <div style={{ fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#444" }}>Add →</div>
                </div>
              ))}
            </div>

            <button style={{ background:GOLD, color:BLACK, border:"none", borderRadius:6, padding:16, width:"100%", fontFamily:"'Barlow Condensed', sans-serif", fontWeight:900, fontSize:20, cursor:"pointer", textTransform:"uppercase" }}>
              💰 REQUEST PAYOUT
            </button>
          </div>
        )}
